...
system) are called \emph{metatheorems}. Type safety\footnote{A result often
  sloganized as \emph{well-typed (closed) terms don't go wrong} or
  \emph{well-typed (closed) terms don't get stuck}, with both slogans meaning
  that it is not possible to reach an irreducible term which is not a chosen
  \emph{value} when reducing starting from a well-typed closed term.} is another
metatheorem about the simply typed lambda calculus which is much more famous
than normalization, perhaps because its proof is so straight forward. Type
...
\emph{mutable reference types} (the effect being change of \emph{state}). For
systems involving such features, even type safety results require the
development of more heavy machinery. In the case of \emph{mutable reference
  types}, the heavy machinery called for is the method of \emph{step-indexed
  logical relations}, a form of \emph{Kripke logical relations} which I describe
below and detail in this chapter. Ahmed, for example, employed
\emph{step-indexed logical relations} to prove the type safety of a version of
System F endowed with mutable reference types.
...
that if a term is in the relation when allowed $k$ steps of reduction is in the
relation when allowed \emph{fewer} steps of reduction of well.

[TODO from here]

The \emph{Artin gluing} construction, torn from the topos
theorist's cookbook, is a more transparent encoding of the mechanics of Kripke
logical relations. Where the method of Kripke logical relations is ad-hoc and
mysterious, gluing regularizes the thought involved and delivers more conceptual
