...
categories of lambda theories satisfying the beta-eta rules. We will not repeat
the proof found in Taylor \cite{taylor_practical_1999}, but it largely amounts
to the replication of the \( \beta \) law as well as the uniqueness requirement in
the universal property of the exponential transpose.
...
\begin{definition}[Algebraic theory of the lambda calculus]
  ...
  \begin{enumerate}
    ...
    \item We define the following operation symbols for each
         ...
         \begin{align*}
           \Gamma &\vdash (\lambda x.\, t)(\overrightarrow{\gamma}) : \tau && \text{(where $\overrightarrow{\gamma}$ is the list of variables comprising $\Gamma$)}
         \end{align*}
          The operation symbols are the least set closed under taking
          abstractions in this way and containing the operation symbols already
          mentioned above.
