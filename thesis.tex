...
related to the method of logical relations, but regularizes some of the
...
monotonicity condition: for any renaming \( \rho : \Gamma' \rightarrow \Gamma \), if \( t \) TODO.
Since we're working in $\catset$, those monomorphisms are the usual injective
functions. To see why this recovers what we had before, we need to look at the
naturality diagram of \( q \):
  ...
  Because they are monos, we may allow ourselves to identify the component
  morphisms with their images. In particular, we write \( |q_{\Gamma}| \subseteq \tm(\Delta)(\Gamma) \)
  for the image for each \( \Gamma \). Now what this diagram says is that for any
  \( q_{\Gamma }(r) = t \in |q_{\Gamma}|\), we have that
