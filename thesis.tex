...
plausible choice for \( \delta' \) is
\[ i_{e} \circ \widetilde{\delta \circ i_{\pi}} \]
...
that it fits into the pullback diagram above. The up-side of our ignorance is
...
that \[ q_{Y}^{*} \circ \reallywidetilde{\tm(\epsilon) \circ i_{\pi}} \circ i_{e} \circ \reallywidetilde{\delta \circ i_{\pi}} \circ q_{X} = {q_{Z}}_{*} \circ \tilde{d} \]
...
that \[ \delta \circ i_{\pi} \circ (q_{X} \times q_{Y}) = q_{Z} \circ d \]. In what follows, we call
this equation the gluing equation.

To untangle this mess, we pass to the lambda notation for the transpose as in
Chapter~\ref{chapter:TLC}. We can express the left-hand side of the desired
equality in this notation (dropping the typing annotations and adding explicit
variables) as
\begin{align*}
  &\lambda y'.\, (\lambda y.\, \delta ((q_{X}(x:R_{x}, y)))) q_{Y}(y') \\
  = &\lambda y.\, \delta (q_{X}(x: R_{X}), q_{Y}(y)) && (\textrm{by the $\beta$ rule}) \\
\end{align*}
where the beta rule holds because \( \renhat \) is cartesian closed. This turns
out to be exactly the transpose of the the left-hand side of the gluing
equation: the transpose is
\[ \reallywidetilde{\delta \circ i_{\pi} \circ (q_{X} \times q_{Y})} \] which can be rendered in the
lambda notation as \[ \lambda y.\, \delta(q_{X}\times q_{Y} (x : R_{X}, y)) \] so that by the
definition of the product functor, the left-hand side of our desired equality is
precisely the transpose of left-hand side of the gluing equation. The right-hand
side of the desired equality is evidently the transpose of the right-hand side
of the gluing equation. Finally, since taking transposes is well-defined, we
...
\begin{align*}
  ...
  \phi_{2}(\delta) &= i_{e} \circ \reallywidetilde{\delta \circ i_{\pi}}
\end{align*}
...
\begin{align*}
  ...
  \psi_{2}(\phi_{2}(\delta)) &= \tm(\epsilon) \circ ((i_{e} \circ \reallywidetilde{\delta \circ i_{\pi}}) \times \id) \\
\end{align*}
...
The second equality is a little more tricky. Both of these are the universal
