...
definition of a concept we will get a lot of mileage out of in this thesis. This
definition should already be familiar from our study of type theory, despite the
...
continue, we should decide what our composition will be.
...
but will prove useful in studying Example \ref{ex:monoid sketch}.
...
\begin{proof}
  ...
  \end{itemize}
p  where \(\sigma : \Gamma, x : X, \text { and }, y:Y\). Covariance of the
...
substance of this covariant action morally amounts to saying that the monoid
acts on itself by left multiplication (here ``multiplication'' is actually just
juxtaposition plus some parentheses) which gives the robust version of the
...
% continue from here; what should we do about the above stripped out stuff?
% should I just skip elementary sketches entirely? the definitions in that
% section definitely help to lay the groundwork for the
...
\end{theorem}
\begin{proof}
  Omitted. We will give the proof for a more general result in Theorem
...
doctrine of elementary sketches and allow us to do so.
\begin{definition}[Algebraic theory]\label{def:alg theory}
  ...
  \begin{enumerate}
    ...
    \item an inexhaustible collecting of variables $x_{i}:X$ of each sort;
...
with another abstract definition: that of an \emph{$\mathcal{L}$-algebra} for ana
algebraic theory:
  ...
  \begin{enumerate}
    ...
    \item We wouldn't dare bore the reader by verifying all the laws, so we
          ...
          \begin{proof}
            ...
            clearing of that remaining goalpost deferred, we have shown what is
...
makes no commentary on terms besides the action on the clones given in Theorem
\ref{thm:clone model}. Our models so far have only given meaning to the
