...
\begin{corrolary}[An interpretation of base types in a CCC gives rise to an interpretation of $\lambda$-terms]
\end{corrolary}
...
\begin{definition}[Gluing syntax and semantics of variables]
  ...
  the components of the quotient are the interpretation of terms in the
  classifying category. This object glues the syntax of a variable together with
  its semantics as substitution for a term of the appropriate type, for each
  variable of type $\tau$.
\end{definition}
...
\( NN \) algebra in \( \renhat \) together with the semantic \( NN \)
algebra\footnote{i.e., the one induced by Remark~\ref{rmk:upgrade_to_stratified}
  and the usual lambda algebra in the classifying category} in the classifying
category.
...
\begin{itemize}
  ...
  \item Idempotency: For all normal terms \( n \in \nfpsh(\Gamma) \),
  \[ \nf_{\tau}^{\Gamma}(n) = n \]
...
[TODO: say here something about how the the proofs of each of these will ``fall
...
this chapter. For each type \( \tau \in \types \), we wish to come up with maps
...
\end{theorem}
\begin{proof}
  The semantic component of the glued maps was defined to be the identity.
\end{proof}
...
\begin{theorem}\label{sem-pi-is-id-endofunctor}
  The composite \( \pi \circ \sem{-} : \cl \rightarrow \cl \) is the identity endofunctor on \( \cl \).
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
  We follow Fiore in offering a sketch (for now: TODO). This result follows by
  Theorem~\ref{thm:reif-refl-sempres} and some consideration about the content
  of the normalization function. The normalization function takes a term
  \( \Gamma \vdash t : \tau \) to its interpretation \( [t/x] \) in the classifying category
  by \( l_{\tau} \), and then interprets the semantics into the gluing category by
  \( \sem{-} : \cl \rightarrow \gl \); finally, the function performs some adjustments by
  way of (products of) \( \downarrow_{\tau}\) and (product of) \( \uparrow_{\tau}\) and then projects
  out the semantic part of the result by the homomorphism \( n_{\tau} \).
  Theorem~\ref{sem-pi-is-id-endofunctor} says that extending the interpretation
  preserves the semantics, and Theorem~\ref{thm:reif-refl-sempres} says that the
  reification and reflection maps don't touch the semantic component. There
  remains a question as to what the map \( v_{\Gamma} \) does, which Fiore doesn't
  address, TODO. But otherwise, that is just about the proof. Also, can I
  actually say that \( n_{\tau} \) projects out the semantic component?
\end{proof}
