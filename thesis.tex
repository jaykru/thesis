...
\begin{theorem}[Reification and reflection preserve semantics]\label{thm:reif-refl-sempres}
  ...
  where \( \pi : \gl \rightarrow \cl \) is the forgetful projection into the semantics.
\end{theorem}
...
desired diagram from section~\ref{sec:taking-stock}. We can now define a normalization function \( \nf_{\tau}^{\Gamma} : \mathfrak{L}_{\tau}(\Gamma) \rightarrow \mathfrak{Nf}_{\tau}(\Gamma)\) as the composite

\[ \mathfrak{L}_{\tau}(\Gamma) \xrightarrow{(\Gamma \vdash t : \tau) \mapsto [t/x]} \cl[\Gamma, \tau] \xrightarrow{\sem{-}} \gl [\sem{\Gamma}, \sem{\tau}] \xrightarrow{(\uparrow_{\Gamma} v_{\Gamma})^{*} \circ {\downarrow^{\tau}}_{*}} \gl [\yoneda(\Gamma), \eta_{\tau}] \xrightarrow[\cong]{(d,\delta) \mapsto d(\id_{\Gamma})} \nfpsh(\Gamma) \]
...
We now establish the various correctness results we hinted at above.
...
\end{theorem}
\begin{proof}
  \( \beta\eta\alpha \)-equivalent terms have the same denotation ??? TODO so that the
  equality follows directly from the definition of the normalization function.
\end{proof}
...
\end{theorem}
\begin{proof}
  Follows by Theorem~\ref{thm:reif-refl-sempres}.
\end{proof}
