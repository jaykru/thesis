...
\usepackage{amsmath} % sensible math notation
\usepackage{braket}
\hypersetup{
  colorlinks,
  allcolors=black,
...
% TODO: should I mention more about how models give rise to a category for a sketch?
...
\begin{example}\label{ex:integer ring}
  \begin{enumerate}
    ...
    % TODO: to really explain what's going on here, I feel like the
    % more categorical formalization of the laws as commutative diagrams
    % preserved by the model functor would be the best presentation. I'm not
    % sure why Taylor didn't do this himself. Would it really be so hard to
    % rewrite my own presentation from that perspective? On the other
    % hand, I think that the more syntax oriented presentation of sketches and
    % algebraic theories is probably more approachable to a general
    % audience (even category theorists.) I.e., the syntactic presentation
    % motivates and clarifies the categorical one, at least in my experience
    % learning the syntactic first...
    \begin{align*}
      +_{A} \circ \braket{0_{A}, \id} &= (x : \{ \star \}, y : \mathbb{Z}) \mapsto 0(x) + y (\text{TODO: finish me!})\\
                                 &= (\_, y) \mapsto 0(\star) + y
                                 &= \id_
    \end{align*}
  \end{enumerate}
...
\begin{theorem}[Extending to a model of terms]\label{thm:term model}
% TODO
\end{theorem}
