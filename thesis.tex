...
\[
  5\, (\lambda y.\, y)
\]
is stuck because we have no clue what to do with a numeral applied to an
abstraction and such a thing isn't one of our chosen values. This example
illustrates that stuck terms are in some sense a coarse approximation for
...
calculus is \emph{not} Turing complete and can only solve \emph{decidable}
problems. This can be a problem for some applications. If you want, say, your
web server to run indefinitely without bound; the simply typed lambda calculus
can't do that. You could, however, make your program loop until the projected
death of the sun and deploy a new version of your server shortly thereafter.
Having noted that there are some workarounds to the problems arising from the
terminating character of the simply typed lambda calculus, it is interesting to
note also that this very character is actually required for some applications;
for some purposes, non-terminating languages have no such cute workarounds.
...
free\footnote{Free variables are those variables appearing in a term which are
  not bound by a lambda abstraction.} variables come from
