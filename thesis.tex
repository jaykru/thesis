...
\begin{remark}\label{rmk:upgrade_to_stratified}
  ...
  lambda theory \( \mathcal{L}_{\alpha\beta\eta}\) gives rise to an algebra $\mathfrak{A}$
  of the theory \( NN \) by setting
  \begin{align*}
...
\begin{definition}[A lambda algebra on the presheaves of open syntax]\label{def:open_syn_algebra}
...
\subsubsection{Variables and binding in Renhat}
...
Note: this is not exactly a lambda-algebra, but really something else because of
the mutual recursion. But I think I should leave this technicality to the end
and get moving towards the result at hand...

\begin{definition}[Renhat-lambda algebra of neutrals and normals]\label{def:renhat-lambda-algebra-neut-norm}
  TODO: in notebook mostly. this is actually a MN-algebra, for the theory MN
  which sort of generalizes the usual lambda theory by throwing in a bunch of
  sorts allowing us to distinguish between neutrals and normals.
\end{definition}

\subsection{Interpreting terms in the gluing category}
...
these glued objects. The interpretation of base types in the syntactic category
gives rise to the following interpretation in the gluing category.

\begin{definition}[Lambda algebra in the gluing category]\label{def:lambda-algebra-neut-norm}
\end{definition}
