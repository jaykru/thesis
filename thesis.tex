...
\chapter{Functorial semantics: sketches and their models; algebraic theories and their algebras}\label{chapter:functorial_semantics}
...
getting exactly that original term. Freedom comes from the fact that the sketch
has no equations.
...
moreover induce an isomorphism.
...
operations\footnote{Some readers may appreciate that the doctrine of algebraic
  theory can be rephrased in terms of operards: algebraic theories are operads
  for which the tensor product used in forming the operation domains happens to
  be the (Cartesian-style) product~\cite{TODO: Nlab}}. Algebraic theories
generalize the doctrine of elementary sketches and allow us to do so. As we
upgrade our doctrine to allow products, many of the notions (\emph{terms,
  clones, syntactic category, etc.}) which we developed in the simplified world
of elementary sketches will come along for the ride.
...
Most things you'd like to prove about a type theory are concerned with the
\emph{terms} of the theory at hand. Normalization theorems talk about the
accessibility (under some reduction relation) of a certain class of terms from
any arbitrary open term. \emph{Canonicity} theorems talk about the accessibility
of a different class of terms from arbitrary \emph{closed} terms. These are but
two examples of a broad spectrum of properties one might desire of the terms of
a theory. Considering the primacy of term properties in type theory, it is
rather strange that the notion of semantics we have built so far makes no
commentary on terms besides the action on the clones given in
...
extend canonically \emph{along the product structure} to contexts and
substitutions and thus give meaning to terms.
...
\begin{definition}[Raw cartesian closed structure]
  ...
  \begin{enumerate}
    ...
    \item For each object \( \Gamma \), a function of hom-sets
          ...
          abstraction. Informally, this is a good expression to have in mind:
          \( \mathfrak{u}^{*}(\lambda x\, . p) = \lambda x.\, (u^{*}p) \).
  \end{enumerate}
\end{definition}
...
construct and interpretation of terms from an algebra in a raw cartesian closed
category. The essence is that we extend the assignments of base types and
operation symbols along products as before and along the new exponential
objects as well.
...
It turns out that all of these examples of
cartesian closed categories can also be characterized as the classifying
categories of raw cartesian closed theories that satisfy the beta-eta rules. We
will not repeat the proof found in Taylor \cite{taylor_practical_1999}, but it
largely amounts to the replication of the \( \beta \) law as well as the uniqueness
requirement in the universal property of the exponential transpose.
...
\begin{theorem}
  The classifying category \( \cl \) of the theory \( mathcal{L}_{\lambda\alpha\beta\eta} \) is
  cartesian closed and has a lambda algebra. Further, if $C$ is any cartesian
  closed category with a lambda algebra then there is a unique functor
  \( \sem{-} : \cl \rightarrow C \) that preserves the cartesian closed structure and the
  lambda algebra.
\end{theorem}
\begin{proof}
...
In order to motivate the next, example of a comma construction, we will
introduce the notion of \emph{variable-arity Kripke relations}. Variable-arity
Kripke relations are sort of like relations parameterized by the objects of some
category for which inclusion in a relation respects, in a sense to be defined,
the morphisms of that category.
...
relations to \emph{proof relevant logical predicates}. Indeed, there is a a
category of $C$-Kripke relations whose objects are defined as above; that
...
cases to undaerstand it. Consider any singleton context \( \tau : \cl \). $\tm$
...
essential to recovering the monotonicity condition. To see this, we
need to look at the naturality diagram of \( q \):
  ...
  condition for $\mathfrak{C}$-Kripke relations: containment in each predicate
  is functorial with respect to renaming \emph{up to renaming}. It turns out
  that the category of \(\ren\)-Kripke relations is a full subcategory of the
  gluing category \( \renhat \downarrow \tm \).
...
\begin{remark}[Why the syntactic category just won't do]\label{rmk:syntactic-cat-bad}
  ...
  function of sets in the family going in the opposite direction. If we take the
...
For all this talk of normalization, we haven't said much about what a
normalization function must be. Of course, it should take a lambda term to a
normal form, but the desiderata are far more than that. The following are some
of the important properties a normalization function
...
\begin{theorem}[Relative hom functor is cartesian closed]
  ...
  in full. It should basically follow from the same result for Yoneda embedding.
  It also requires us to state the cartesian closed structure of the categorya
  \( \renhat \), which is well-known but should be stated nonetheless.
\end{theorem}
...
interpretation defines a lambda algebra of glued neutrals. Initiality of the
classifying category and its lambda algebra in turn induces a unique functor
\( \sem{-}_{\gl} : \cl \rightarrow \gl \) essentially as in Theorem~\ref{thm:classifying
  alg theory}. We write \( \sem{-}_{\gl} : \cl \rightarrow \gl \) for both this functor
and the interpretation of terms acquired by precomposing the functor with the
usual interpretation of terms in the classifying category. It is crucial to
understand that though we have \( \sem{\theta}_{\gl} = \mu_{\theta}\) for base types $\theta$,
this equality does \emph{not} hold at higher types \( \tau \in \types\). In the case
of products, the interpreted type \( \sem{\theta \times \theta'}_{\gl} \) is the actual product
...
initiality of the classifying category.
...
\subsection{Glued reification and reflection}\label{def:reify-reflect}
...
\cite{fiore_semantic_2002}, and the proof is our own.
