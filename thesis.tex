...
% TODO: need to have some corrolary or theorem saying how the classifying
...
(cite TAPL). The method of logical relations is, broadly speaking, opaque even
to experienced practitioners. The construction is easy to use in simple settings
like that of the simply-typed lambda calculus, but becomes devilishly
complicated when working with more complex lambda calculi.
...
\newcommand{\gluingcat}{\textrm{Gl}_\Sigma}

We now define the \emph{gluing category} as the comma of the category of
renamings along the relative hom functor just defined.

\newcommand{\gl}{\textrm{Gl}_\Sigma}
...
\begin{definition}[The gluing category]
  ...
  clear after the next subsection. The morphisms $(d : D \rightarrow D', \delta: \tm(\Delta) \rightarrow
  \tm(\Delta'))$ TODO: finish
\end{definition}
...
distinguish between \emph{neutral terms}, which are in some sense beta-redexes
whose reduction is \emph{blocked} by a variable in the head term.
...
\begin{definition}[Neutral and normal judgments]
  \begin{mathpar}
    ...
    \inferrule[]{ }{ \Gamma, x : \tau, \Gamma' \vdash_\neu x : \tau } \and \inferrule{ \Gamma \vdash_\neu M : \tau_1 \times \tau_2}{\pi_1 M : \tau_i} \\
    \inferrule{ \Gamma \vdash_\neu t_{1} : \tau' \rightarrow \tau  \\  \Gamma \vdash_\nf t_{2} : \tau' }{ \Gamma \vdash_\neu t_{1} t_{2} : \tau } \and \inferrule{}{ \Gamma \vdash_\nf () : \termob }\\
    \inferrule{\Gamma \vdash_\nf N_i : \tau_i}{\Gamma \vdash_\nf (N_1, N_2) : \tau_1 * \tau_2} \\
    \inferrule{\Gamma, x : \tau \vdash_\nf b : \tau' }{\Gamma \vdash_\nf \abstr{x}{\tau}{b} : \tau \rightarrow \tau'} \\
    \inferrule*[Right={ (\theta \in T \textrm{a base type}) }]{\Gamma \vdash_neu t : \theta}{\Gamma \vdash_\nf t : \theta} \\
  \end{mathpar}
  \end{definition}
...
\begin{definition}[Algebraic theory of neutrals and normals]\label{def:theory_neut_norm}
  ...
  We define the lambda theory \( NN \) as follows:
  \begin{enumerate}
    \item Sorts:
    \( \Sigma = \{ \mathcal{M}_\tau \mid \tau \in \types \}
    \cup \{ \mathcal{N}_\tau \mid \tau \in \types \}
    \cup \{ \mathcal{V}_\tau \mid \tau \in \types \} \)
    \item Operation symbols:
    \begin{align*}
    ...
    \item The laws are $\alpha\beta\eta$.
  \end{enumerate}
  Note that the codomain of the unit operation symbol is the nullary product of
  sorts.
\end{definition}

\begin{remark}\label{rmk:upgrade_to_stratified}
  Any algebra \( \{ (\mathfrak{V}, \mathfrak{X}_{\tau}) \}_{\tau \in \types}\) of the
  lambda theory \( \mathcal{L}_{\alpha\beta\eta}\) from Chapter~\ref{chapter:stlc} gives
  rise to an algebra $\mathfrak{A}$ of the theory \( NN \) by setting
  \begin{align*}
...
\begin{remark}[A lambda algebra on the presheaves of open syntax]\label{def:open_syn_algebra}
  The presheaves of open syntax \( \mathfrak{L}_{\tau}\) induce an algebra for the
  lambda theory \( \mathcal{L}_{\alpha\beta\eta} \) defined in
  \ref{def:alpha_beta_eta_theory} TODO
\end{remark}
...
\begin{remark}[interpretations]
  TODO give the diagram of interpretations
...
Where step (2) follows by the definition of the exponentials in the category of
presheaves, c.f. page 46 of \cite{mac_lane_sheaves_1992}.

The presheaves of typed variables together with the simplified view of
exponentials in $\renhat$ allow us to define an algebra for the lambda theory
\( NN \) of stratified neutrals and normals defined in the previous section.

\subsection{A lambda-algebra of neutrals and normals in Renhat}
\begin{definition}[A stratified lambda-algebra of neutrals and normals]\label{def:strat-lam-alg}
  ...
  The operations are given by TODO
\end{definition}
...
\begin{theorem}[Exponentials in the gluing category]
  ...
  is \( (R, q, \Delta) \) in the pullback diagram
  \[\begin{tikzcd}
      R && {{R_2}^{R_1}} \\
      \\
      ...
      \arrow["\scalebox{2.0}{\( \lrcorner \)}", from=1-1, to=2-2, very near start, phantom]
    \end{tikzcd}\]
...
\begin{align*}
  \psi_{2}(\phi_{2}(\delta)) &= \tm(\epsilon) \circ i_{e} \circ \widetilde{\delta \circ i_{\pi}} \\
...
\begin{definition}[Gluing syntax and semantics of variables]
  We define the glued object \[ \nu_{\tau} = (\varpsh, \varpsh \rightarrow \tm(\tau), \tau)\] where
  the components of the quotient are the interpretation of the term syntax in
  the classifying category. This object glues the syntax of a variable together
  with its semantics as substitution for a term of the appropriate type, for
  each variable of type $\tau$.
\end{definition}
...
\begin{definition}[Gluing syntax and semantics of neutrals]
  \[ \mu_{\tau} = (\neut, \neut \rightarrow \tm(\tau), \tau) \]
\end{definition}

\begin{definition}[Gluing syntax and semantics of normals]
  \[ \eta_{\tau} = (\norm, \norm \rightarrow \tm(\tau), \tau) \]
\end{definition}

With these glued objects, we can define a lambda algebra of glued variables,
neutrals, and normals. In turn Definition~\ref{def:term model} will allow us to
leverage this to interpret substitutions as gluing category morphisms between
these glued objects. We can now glue the syntactic \( NN \) algebra in
\( \renhat \) together with the semantic \( NN \) algebra\footnote{i.e., the one
  induced by Remark~\ref{rmk:upgrade_to_stratified} and the usual lambda algebra
  in the classifying category} in the classifying category to get an \( NN \)
algebra in \( \renhat \downarrow \tm \).

\begin{definition}[An algebra of stratified neutrals and normals in the gluing category]\label{def:lambda-algebra-neut-norm}
\end{definition}
...
Let \(s\sem{-} : \types \rightarrow \cl \) be the interpretation of types as contexts in
the syntactic category and terms as substitutions. By (TODO: $\pi_2$ second
component gluing projection), the interpretation of terms induced by
\( \bar{s} \) according to Definition~\ref{def:term model} extends the
interpretation of terms in the syntactic category in the following sense:
\( \bar{s}\sem{\Gamma \vdash t : \tau} \) is a pair of the form
$(s'\sem{ \Gamma \vdash t : \tau}, s\sem{\Gamma \vdash t : \tau })$ where \( s' : \cl \rightarrow \renhat \) is the
unique functor induced by the $\renhat-$lambda algebra of neutral and normals
from Definition~\ref{def:lambda-algebra-neut-norm} and the cartesian closed
structure of the category of presheaves. Now is a good time to take stock of
what we have developed so far, and look ahead to where we're going.


\subsection{Taking stock: charting a path to normalization}
Begin by noting that we can consider \( \tm : \cl \rightarrow \renhat \) as an
\emph{interpretation} of the base types of the lambda-theory in \( \renhat \).
We now have enough language to say exactly what it is we're hoping to acquire in
this chapter. For each type \( \tau \in \types \), we wish to come up with maps to
fill in the dotted arrows in the upper diagram such that the lower diagram
commutes. In particular, the composite along the top edge going down to the
bottom right corner will be our desired normalization function.
...
\chapter{Orphaned snippets}

This is where things without a home in their chapter go\ldots for now.
