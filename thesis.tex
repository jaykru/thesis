...
more famous model of universal computation, are equivalent \emph{in
  computational power}: any Turing machine can be simulated by the lambda
calculus, and consequently any decidable problem can be decided by the lambda
calculus.
...
is the famed \emph{type safety} property from programming language theory.
Another family of properties, namely \emph{normalization} properties, follows
from this type regime. It turns out that any program in the lambda calculus will
reduce to a value in a finite number of evaluation steps. This property is
called \emph{closed normalization}. In the language of Turing machines, this
means that programs written in the simply typed lambda calculus always halt. In
particular, the lambda calculus is \emph{not} Turing complete and can only
...
thereafter. Beyond silly considerations like this, the halting property of
languages like the simply typed lambda calculus is actually required for some
applications.
...
treat these topics in this thesis, but such a document would have been perhaps
...
gluing construction casts the normalization problem
...
the result is open normalization means that we are giving a term reducer]
