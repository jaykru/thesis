...
%\usepackage{unicode-math} % TODO: $this is incompatible with Coloneqq for some reason, but I think I need it for multi-letter mathcal :( need to fix!
...
% I'd also like to give special thanks to Albyn Jones and Irena Swanson who guided
% my early explorations of mathematics. Having been brought from complete
% ignorance to a great appreciation for proof and mathematical exploration under
% their careful guidance was a great privilege! I am also grateful to Kyle Ormsby
% for always being an absolute chiller and encouraging me in early ambitions.
...
understand that though \( \sem{\theta}_{\gl} \) and \( \mu_{\theta}\) are the same for base
types $\theta$, but are \emph{not} the same for higher types \( \tau \in \types\). In the
case of products, the interpreted type \( \sem{\theta \times \theta'}_{\gl} \) is the actual
product \( \mu_{\theta} \times \mu_{\theta'}\) and not \( \mu_{\theta\times\theta'} \). In what follows, we still
allow ourselves to write \( \sem{-} \) for the usual interpretation of terms in
the classifying category, distinguishing the new interpretation only by a
subscript. The induced interpretation \( \sem{-}_{\gl}\) extends the usual
interpretation of terms in the syntactic category in the following sense:
  ...
  equality.
    \begin{itemize}
      \item For a base type \( \theta \in T \), we have \( \Uparrow^{\theta} = \id_{\mu_{\theta}} \) and
            \( \Downarrow^{\theta} = \mu_{\theta} \xrightarrow[\cong]{\incl_{\theta}} \eta_{\theta} \). The former is
            the identity which is a map in the gluing category by definition.
            The latter is a glued map from Definition~\ref{def:glued-algebra}.
      \item For the empty product/unit type \( \termob \), we have
            ...
            The former is a glued map as guaranteed by the universal property of
            the terminal object in the gluing category. The latter is a glued map from Definition~\ref{def:glued-algebra}.

      \item For types \( \tau, \tau' \in \types\), we have that
            \[ \Uparrow^{\tau*\tau'} : \mu_{\tau*\tau'} \rightarrow \sem{\tau}_{\cl} \times \sem{\tau'}_{\cl}\] is the
            product of the following composites:
            \begin{align*}
              &\mu_{\tau*\tau'} \xrightarrow{(\fst_{\tau}^{\tau'}, \pi_{1})} \mu_{\tau} \xrightarrow{\Uparrow^{\tau}} \sem{\tau}_{\gl} \\
              &\mu_{\tau*\tau'} \xrightarrow{(\snd_{\tau'}^{\tau}, \pi_{2})} \mu_{\tau'} \xrightarrow{\Uparrow^{\tau'}} \sem{\tau'}_{\gl} \\
            \end{align*}
