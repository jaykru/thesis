...
thank, whether for their direct impact in making this thesis possible or just
for having had an outsized impact on my life at one point or another. I can only
start by by thanking my advisors Ang\'elica and Jim for taking my enthusiasm to
learn modern approaches to type theory and helping me channel it productively. I
am indebted to Ang\'elica's spooky ability to instantly grasp at understanding
in an area relatively unknown to her. Without her hints at how to tackle an
opaque construction every now and then, I think I'd probably be still fumbling
around with the basics. Jim has had an outsized direct influence on the
direction my intellectual life has taken. Jim's intro CS course taught partially
in Standard ML was my first real exposure to mathematically structure
programming and opened me up to a world that had until then had seemed
inaccessible to me. Jim has also served as a great mentor in matters of personal
taste and style (his course webpages are always out of this world cool.) I'll
really miss thesis meetings with you two!

Next I have to thank the rest of the faculty and students in the Mathematics
department who have made the last few years such an enriching experience. Albyn
Jones and Irena Swanson, who taught my intro courses freshman year, are singled
out for special thanks. Having been brought from complete ignorance to a great
appreciation for proof and mathematical exploration under their careful guidance
was a great privilege! I am also grateful to Kyle Ormsby for always being an
absolute chiller, encouraging me in early ambitions, and for giving me a taste
of category theory before I fled from his topology class.
...
I would like to thank my parents for having let me be and encouraging me to
grow into the person I am today. That is a rare thing, and has made all the
difference to me!
...
I would like to thank the following teachers in my life. My first grade teacher
Ms.\ King, 柳老師 (Hyong Rhew) for getting me to chill out a bit freshman year,
Mrs.\ Leitsch for extensive encouragement in 9th grade English, Mr. Cool for
encouragement on the robotics team (even if you couldn't get the team to switch
from LabView), Mr.\ Raveli for reassuring me that it's okay to not know all the
answers, Shulav and Aditya for many fine hours of good lad time.
...
wide tildes macro, and to user13907 for their stylized B symbol.
...
\begin{example}[Algebraic theory of \emph{ring}]\label{ex:theory of ring}
  ...
  \begin{enumerate}
    \item Sorts: The only sort is \( S \). The variable collection for the sort
          is \( \{x,y,z\} \cup \set{s_{i}}_{i\in\mathbb{N}} \).
  ...
  are added so that we may have deeply nested expressions, even if we had no
  need for them when stating the laws above. In fact, such countable variable
  sets are \emph{required} when defining an algebraic theory for precisely this
  reason. That this requirement was not imposed for elementary sketches is
  because there can only be one variable in a term, even in heavily nested terms,
  by construction.
\end{example}

The following example is meant for readers with a background in computer
science, and in particular in digital logic. An interesting aspect of this
example is that it admits some very practical algebras.

\begin{example}[The algebraic theory of digital logic circuits]
...
A mundane algebra for this theory is given as follows: We assign the the single
sort to be the set \( \{0,1\} \). For the operation symbols, we assign \( \top \)
to the function \( * \mapsto 1\); \( \bot \) to function \( * \mapsto 0 \); $\texttt{AND}_{k}$
to the function whose value is $1$ iff its input bitstring (i.e., tuple of ones
and zeroes) contains only ones and $0$ otherwise; and $\texttt{OR}_{k}$ to the
function whose value is $0$ iff its input bitstring contains only zeroes and $1$
otherwise.
...
NOT, AND, and OR as component circuits built with transistors. The exact details
of how semiconductors work are beyond the scope of this thesis; it suffices to
understand that a transistor is an electrical device which allows current to
flow conditionally. Below is a diagram of a \emph{metal oxide semiconductor}
field effect transistor (MOSFET). You probably own a couple billion of them.
...
The conditional flow of current works like this: if there is a current flowing
from \emph{gate} input (labeled G below) to the \emph{drain} input (labeled S
below), then the transistor is \emph{on} and current may also flow from the
\emph{source} to the \emph{drain} output. We can use this conditional flow
behavior to implement each of the operations of the theory presented above. We
will consider the simplest case of the NOT operation and leave the others (as
well as the full details) to the experts~\cite{eater_making_2015}. The NOT
operation is given by an \emph{inverter} circuit which is easily constructed
using a single MOSFET.\@ Abstracting the details a bit the construction goes
something like this: the single input to the NOT gate circuit is attached to a
switch which when closed allows current to flow from the \emph{gate} to the
\emph{drain}, and the \emph{source} and \emph{drain} are involved in a circuit which always has current in it when the transistor is on.

  \iem the \emph{source} of the
    transistor to a wire that always sees high voltage, and, finally, the
\emph{drain} of the transistor serves at the output for the NOT circuit.
\end{enumerate}
 When
the input line is unset, the output is set; when the input is. Having given some
concrete examples, we now allow ourselves another abstract definition: that of
an \emph{$\mathcal{L}$-algebra} for an algebraic theory:
