...
\newcommand{\evsig}[2]{[{#2^{#1}}, #1] \vdash \textrm{ev}_{#1,#2}(f,x) : #2}
...
\begin{definition}[Lambda theory] % TODO: is this actually a lambda-theory?
  ...
  % to the category...
  A \emph{lambda theory} is an algebraic theory \( \mathcal{L} \) with, for each
  sort \(X, Y \in \Sigma\):
  \begin{enumerate}[label=\alph]
    \item A sort \( Y^{X} \)
    \item An operation symbol, \emph{application}, \( \evsig \) and
    \item For each list of sorts \( \Gamma \), a function of operation symbols
          \( \lambda_{\Gamma, X, Y} : \cn_{L}(\Gamma \times X, Y) \rightarrow (\Gamma, Y^{X})\) obeying the
          naturality law:
          \[ \lambda_{\Gamma, X, Y} (\mathfrak{p} \circ (\mathfrak(u) \circ \id_{X})) = (\lambda_{\Gamma, X, Y}(\mathfrak{p}) \circ \mathfrak{u})\]
