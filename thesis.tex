  ...
  \begin{itemize}
    ...
    \item For \( \tau, \tau' \in \types\), the reflection map \( \uparrow^{\tau\times\tau'}\) for
          ...
          \[ \mu_{\tau\rightarrow\tau'} \times \sem{\tau}_{\gl} \xrightarrow{\id \times \downarrow^{\tau}} \mu_{\tau \rightarrow \tau'} \times \eta_{\tau} \xrightarrow{（\app_{\tau'}^{\tau}, \epsilon）} \mu_{\tau'} \xrightarrow{\uparrow^{\tau'}} \sem{\tau'}_{\gl}\] whence
            ...
            \begin{align*}
              \semproj (\uparrow^{\tau \rightarrow \tau'}) &= \semproj (\widetilde{\uparrow^{\tau} \circ（\app_{\tau'}^{\tau}, \epsilon）\circ (\id \times \downarrow^{\tau})}) \\
                                    &= \widetilde{\semproj (\uparrow^{\tau}) \circ \semproj (\app_{\tau'}^{\tau},\epsilon) \circ \semproj (\id \times \downarrow^{\tau})} && \textrm{(by functoriality of $\semproj$)} \\
                                    &= \widetilde{\id \circ \epsilon \circ (\id \times \id)} && \textrm{(induction hypothesis, definition of $\semproj$)} \\
                                    &= \widetilde{\id \circ \id} && \textrm{(UP of the transpose, and since $\tilde{\id} = \id$)} \\
                                    &= \widetilde{\id} \\
                                    &= \id \\
