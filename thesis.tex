...
\begin{definition}[Algebraic theory of neutrals and normals]\label{def:theory_neut_norm}
  The judgments above suggest the definition of a lambda theory with a richer
  sort structure than that of the lambda theory defined back in Chapter
  \ref{chapter:stlc}. In particular, the new sort structure will feature both a
  neutral sort and a normal sort for each type \( \tau \), allowing us to define
  algebras that distinguish between these.

  We define the lambda theory \( \NN \) as the algebraic theory  corresponding (c.f.
  Theorem~\ref{thm:canonical_elementary_language}) to the category
...
\begin{remark}[A stratified-lambda algebra on the presheaves of open syntax]
  By Remark~\ref{rmk:upgrade_to_stratified}, the \( \mathcal{L}_{\lambda\alpha\beta\eta}\) algebra
  of Definition~\ref{def:open_syn_algebra} induces an algebra of the lambda
  theory \( \NN \).
\end{remark}
...
\]
in the gluing category. We will set \( \downarrow^{\tau} = \Downarrow^{\tau}\) and \( \uparrow^{\tau} = \Uparrow^{\tau}\).
Finally, the desired triangles above follow from demonstrating that the
intermediate maps project in the semantics onto the identity:
\( \semproj(\Downarrow^{\tau}) = id_{\sem{\tau}} = \semproj(\Uparrow^{\tau})\). This follows by
...
\[\begin{tikzcd}
	...
	\arrow["{r_\tau}"{description}, from=1-3, to=3-3]
\end{tikzcd}\]
...
\subsection{Introducing Elmer's Reify-reflect}
\begin{definition}
