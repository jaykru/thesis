...
\begin{definition}[Types]\label{def:stlc_types}
  ...
  \]
  where \( \theta \) is drawn from a collection of \emph{base types} (say natural
  numbers, integers, booleans, or some other type of object the reader is
  interested in.) Note that the unit type \( termob \) is not a base type but
  rather can be considered a nullary product of types.
\end{definition}
