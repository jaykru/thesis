...
\newcommand{\evsig}[2]{[f : {#2^{#1}}, x : #1] \vdash \textrm{ev}_{#1,#2} (f,x):#2}
\newcommand{\ev}[2]{\textrm{ev}_{#1,#2}}
Most sources define this notion en-passant, if at all. We give an explicit
characterization of \emph{lambda theories} for the reader who like the author is
a 5head.

\newcommand{\cnprod}{\cn_{\mathcal{L}}^{\times}}
\begin{definition}[Lambda theory] % TODO: is this actually a lambda-theory?
  % Should I instead call it a theory with raw cartesian-closed sturcture?
  % Perhaps instead it is better to follow Taylor and begin with the category of
  % the algebraic theory, and then characterize raw CC structure with reference
  % to the category...
  Let \( \mathcal{L} \) be an algebraic theory and \( \cnprod \) its classifying
  category. Then \( \mathcal{L} \) is a \emph{lambda theory} if for
  object \(X, Y\) of \( \cnprod \) we have
  \begin{enumerate}
    \item An object \( Y^{X} \) of \( \cnprod \)
    \item An morphism symbol, \emph{application}, \( \evsig{X}{Y} \) and
    \item For each list of sorts \( \Gamma \), a function of operation symbols
          \( \lambda_{\Gamma, X, Y} : \cn_{\mathcal{L}}(\Gamma \times X, Y) \rightarrow (\Gamma, Y^{X})\) obeying the
          naturality law:
          \[
