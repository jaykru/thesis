...
\begin{definition}[Algebraic theory of neutrals and normals]\label{def:theory_neut_norm}
  ...
  We define the lambda theory \( NN \) as the canonical language (c.f. TODO) of the category
...
\begin{remark}\label{rmk:tm-nn-alg}
  The usual lambda algebra on the syntactic category induces an \(NN\)-algebra
  on \( \tm \). TODO: flesh out.
\end{remark}
...
Definition~\ref{def:lambda-algebra-neut-norm} and the cartesian closed structure
...
Begin by recalling that we can consider \( \tm : \cl \rightarrow \renhat \) as an
...
desired diagram from section~\ref{sec:taking-stock}. We can now define a normalization function \( \nf_{\tau} : \mathfrak{L}_{\tau} \rightarrow \mathfrak{Nf}_{\tau}\) as the composite TODO

\[ \mathfrak{L}_{\tau} \xrightarrow{\bar{l}_{\tau}} \tm(\tau) \xrightarrow{} \]
